    ...
    <div class="w-11/12 flex justify-around grow">
        {#each {length: batiment?.nb_door} as _, i}
            <div class="flex flex-col justify-center items-center text-center">
                {#if isInside}
                    <BadgeReaderComponent idHuman="humanInBatiment" idBatiment="{batiment?.id}" idDoor="{i}" bind:canEnter={canPass} bind:doorSeleded={doorSelected}/>
                {/if}
                <LightComponent badgeScanned="{doorSelected === i && isInside}" canEnter="{canPass}"/>
                <div class="flex justify-center items-center">
                    <span class="h-24 w-24">
                        {#if canPass && doorSelected === i}
                            {@html openDoor}
                        {:else}
                            {@html closeDoor}
                        {/if}
                ...
                <LightComponent badgeScanned="{doorSelected === i && !isInside}" canEnter="{canPass}"/>
                {#if !isInside}
                    <BadgeReaderComponent idHuman="humanOutBatiment" idBatiment="{batiment?.id}" idDoor="{i}" bind:canEnter={canPass} bind:doorSeleded={doorSelected}/>
                {/if}
            </div>
        {/each}
    </div>
